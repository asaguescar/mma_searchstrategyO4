...
def parse_commandline():
    ...
    parser.add_option("-p", "--possisDir", help="possis directory",default="kilonova_models/bulladynwind")
    ...
    parser.add_option("--start_time", help="start time.", default='2009-04-25T08:18:25')
    ...
    parser.add_option("--mdyn", help="Mass dynamical ejecta.", default=0.005, type=float)
    parser.add_option("--mwin", help="Mass disk wind.", default=0.05, type=float)
...
if opts.observation_type=='obsfile_noccdid':
    ccd_corners = os.path.join(inputDir,'ZTF_corners.txt')
else:
...
if not os.path.isfile(pcklFile):
    if opts.observation_type == "TAP":
        ...
        if opts.observations[-4:]=='.csv':
            ...
            obstable['ccdid']=np.floor(obstable['chid']/4).astype(int)
        else:
            ...
            obstable.rename_columns(names, renames)
    elif opts.observation_type == "obsfile_noccdid":
        if opts.observations[-4:]=='.csv':
            obstable = pd.read_csv(opts.observations)
            #obstable['ccdid']=np.floor(obstable['chid']/4).astype(int)
            obstable.rename(columns={'time':'jd',
                                     'success':'status',
                                     'limmag':'limMag',
                                     'filt':'filter'}, inplace=True)
        else:
            obstable = Table.read(opts.observations, format='ascii.fixed_width',
                                data_start=2, data_end=-1)
            obstable['ccdid']=np.floor(obstable['rcid']/4).astype(int)
...
for field_id in df.fieldid:
    ipix_list = np.array(ipixs[field_id])
...
if opts.observation_type=='obsfile_noccdid':
    for ii, fieldid in enumerate(df.fieldid):
        probarray.append(probs[fieldid])
        ipixarray.append(ipixs[fieldid])
else:
...
if opts.doSimSurvey:
    ...
    if opts.observation_type=='obsfile_noccdid':
        fields, ccds = load_fields_ccd(survey_fields,ccd_corners, num_segs=16)
    else:
        fields, ccds = load_fields_ccd(survey_fields,ccd_corners, num_segs=64)
    limMags = []
    ...
    mjd_range = (plan.pointings['time'].min()-1, plan.pointings['time'].min())
    ...
    if opts.modelType == "Bulla":
        ...
        if opts.doColor:
            ...
            phase, wave, flux = tophat_template(mag0=opts.mag, dmag=opts.dmag)
...
if opts.doFilter:
    ...
    if opts.doPlots:
        ...
        if lcs.meta is not None:
            ...
            plt.hist(lcs.stats['p_det']+lcs.meta['t0'])
            plt.hist(np.array(p_det_)+np.array(t0_), label='filtered_kNe')
            plt.legend(loc=0)
            plt.xlabel('Detection date (d)')
            plt.ylabel('N')
            plt.tight_layout()
        ...
        if (opts.modelType=='Bulla' or opts.modelType=='Bulladynwind') and phi!=0 and phi!=90:
            ...
            plt.imshow(H,aspect='auto',interpolation='none', origin='lower', extent=[yedges.min(), yedges.max(), xedges.min(), xedges.max()])
